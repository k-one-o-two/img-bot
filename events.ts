...
import type { Bot, Context, Message } from "node-telegram-bot-api";
...
interface PhotoSubmission {
  ...
  isContest: boolean;
  fileIds: string[]; // largest-size file_id per source message
}
  ...
  const handlePhotoSubmission = async (
    ...
    for (const fileId of submission.fileIds) {
      ...
      const filename = await utils.downloadFile(
        ...
        submission.chatId,
        { isContest: submission.isContest },
      );
    ...
    if (msg.photo) {
      ...
      if (msg.media_group_id) {
        ...
        if (existing) {
          ...
          if (msg.caption && !existing.caption) existing.caption = msg.caption;
