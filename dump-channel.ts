...
interface SavedMessage {
  ...
  reactions: { emoticon: string; count: number }[];
  media: {
    type: string;
    file: string | null;
    mimeType?: string;
...
const dumpRoot = (channelId: string): string =>
  path.join(process.cwd(), "channel_dump", channelId);

const ensureDirs = (
  channelId: string,
): { root: string; mediaDir: string; jsonl: string } => {
...
const getResumeFromId = async (jsonlPath: string): Promise<number> => {
  ...
  for await (const line of rl) {
    ...
    try {
      const parsed = JSON.parse(line) as SavedMessage;
      if (typeof parsed.id === "number" && parsed.id > maxId) maxId = parsed.id;
    } catch {
...
const peerToString = (peer: any): string | null => {
  if (!peer) return null;
  if (peer.userId != null) return `user:${peer.userId.toString()}`;
  if (peer.chatId != null) return `chat:${peer.chatId.toString()}`;
  if (peer.channelId != null) return `channel:${peer.channelId.toString()}`;
  return null;
};

const inferExtension = (media: any): string => {
  ...
  if (media instanceof Api.MessageMediaDocument) {
    ...
    if (mime) {
      ...
      if (guess) return guess.split(";")[0];
    }
  }
  return "bin";
};

const summarizeMedia = (media: any): SavedMessage["media"] => {
  if (!media) return null;
  const summary: SavedMessage["media"] = {
  ...
  if (media instanceof Api.MessageMediaDocument) {
    ...
    if (doc) {
      ...
      if (nameAttr?.fileName) summary.fileName = nameAttr.fileName;
      ...
      if (videoAttr) {
        if ("duration" in videoAttr) summary.duration = videoAttr.duration;
        if ("w" in videoAttr) summary.width = videoAttr.w;
        if ("h" in videoAttr) summary.height = videoAttr.h;
      }
    }
  }
...
const shouldDownload = (media: any): boolean => {
...
const resolveChannel = async (
  ...
  for (const build of attempts) {
    try {
      const value = build();
      const entity = await client.getEntity(value as any);
      return entity;
    } catch (err) {
      lastErr = err;
    }
  ...
  throw new Error(
    `Could not resolve channel ${rawId}. Make sure the account is a member of it.\nLast error: ${
...
const run = async (): Promise<void> => {
  ...
  console.log(
    `[dump] resolved: ${(entity as any).title ?? (entity as any).username ?? "(no title)"} ` +
  ...
  try {
    for await (const message of client.iterMessages(entity, {
      ...
      const reactions: { emoticon: string; count: number }[] = [];
      if (msg.reactions?.results) {
        for (const r of msg.reactions.results) {
          const emoticon =
            r.reaction?.emoticon ?? r.reaction?.documentId?.toString() ?? "?";
      ...
      if (count % 25 === 0) {
        ...
        console.log(
          `[dump] saved ${count} messages (last id ${msg.id}) in ${elapsed}s`,
        );
      }
    }
  } finally {
